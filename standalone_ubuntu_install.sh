...
sudo apt -y install nginx-module-image-filter
sudo apt -y install nginx-module-njs
sudo apt -y install nginx-module-xslt
...
required=("S3_BUCKET_NAME" "S3_SERVER" "S3_SERVER_PORT" "S3_SERVER_PROTO" "S3_REGION" "S3_STYLE" "ALLOW_DIRECTORY_LIST" "AWS_SIGS_VERSION")
...
mkdir -p /var/cache/nginx/s3_proxy
chown ubuntu:ubuntu /var/cache/nginx/s3_proxy
